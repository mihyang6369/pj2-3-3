...
# ---------------------------------------------------------
    ...
    render_analysis_box(
        ...
        "싱가포르 노선은 평균 가격대가 약 150만 원 선으로 가장 높게 형성되어 있으며 가격의 편차(Box width)가 매우 커, 초고가 럭셔리 상품부터 합리적 가격대까지 폭폭한 스펙트럼을 보유하고 있음을 보여줍니다. 반면 다낭은 가격대가 50~80만 원 사이에 밀집되어 있어 전형적인 저가 경쟁 시장임을 알 수 있습니다. 특히 주목할 점은 평점 분포입니다. 나트랑은 중앙값 4.5 이상의 매우 고른 만족도를 유지하며 하단 이상치가 적어 안정적인 품질 관리가 이루어지고 있으나, 다낭은 평균 점수 자체가 낮을 뿐만 아니라 평점 1~2점대의 극단적 불만족 사례가 빈번히 관측됩니다. 이는 다낭 지역의 저가 패키지 운영 프로세스 중 현지 가이드 품질이나 쇼핑 일정에서 심각한 만족도 누수(Leakage)가 발생하고 있음을 뜻합니다. 따라서 다낭 지역은 평점 하위 10% 상품에 대한 즉각적인 퇴출(Cut-off) 제도를 도입하고, 나트랑의 안정적 운영 모델을 다낭 지역의 고가 라인업으로 이식하는 전이 전략이 필요합니다."
    )
...
# ---------------------------------------------------------
    ...
    # [추가] 도시별 동행별 평균 리뷰 길이 (텍스트 민감도 분석)
    ...
    # [추가] 도시별 쇼핑횟수별 평균가격과 평점 (이중축 매트릭스)
...
# ---------------------------------------------------------
    ...
    if neg_mining['total'] > 0:
        st.markdown("#### 🧨 부정 리뷰 원인 심층 마이닝 (NLP 분석)")
    ...
    st.markdown("#### 📈 도시별 월별 평균 평점 추이 (미세 품질 변동성 분석)")
    ...
    fig_ct_trend.update_layout(yaxis=dict(range=[4.0, 5.0])) # 평점 4점 이상부터 상세 시각화
